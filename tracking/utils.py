...
def split_dataset(src_fldr: Path, percent_to_delete: float = 0.5) -> None:
    ...
    for seq_path in seq_paths:
        ...
        print(
            f"Number of annotated frames in {seq_path}: Keeping from frame {split + 1} to {nr_seq_imgs}"
        )
...
def download_mot_dataset(val_tools_path, benchmark, max_retries=5, backoff_factor=2):
    ...
    while retries <= max_retries:
        try:
            ...
            if (
                response and response.status_code == 416
            ):  # Handle "Requested Range Not Satisfiable" error
                LOGGER.info(f"{benchmark}.zip is already fully downloaded.")
                return zip_dst
    ...
    if results.size != 0:
        if isinstance(results, np.ndarray):
            ...
            frame_indices = torch.full(
                (num_detections, 1), frame_idx + 1, dtype=torch.int32
            )
