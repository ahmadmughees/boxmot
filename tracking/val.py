...
def cleanup_mot17(data_dir, keep_detection="FRCNN"):
    ...
    unique_sequences = set(
        seq.split("-")[0] + "-" + seq.split("-")[1] for seq in all_dirs
    )
    ...
    for seq in unique_sequences:
        ...
        if os.path.exists(keep_dir):
            ...
            print(
                f"Directory for {seq} with {keep_detection} detection does not exist. Skipping."
            )
...
def prompt_overwrite(path_type: str, path: str, ci: bool = True) -> bool:
    ...
    if ci:
        LOGGER.debug(
            f"{path_type} {path} already exists. Use existing due to no UI mode."
        )
        return False
        ...
        for i, mot_folder_path in enumerate(mot_folder_paths):
            ...
            if dets_path.exists() and embs_path.exists():
                if prompt_overwrite("Detections and Embeddings", dets_path, opt.ci):
                    LOGGER.debug(
                        f"Overwriting detections and embeddings for {mot_folder_path}..."
                    )
                else:
                    LOGGER.debug(
...
def run_generate_mot_results(
    ...
    for y in opt.yolo_model:
        ...
        dets_file_paths = sorted(
            [
                item
                for item in dets_folder.glob("*.txt")
        ...
        embs_file_paths = sorted(
            [
                item
                for item in embs_folder.glob("*.txt")
                if not item.name.startswith(".") and item.stem in mot_folder_names
            ]
        )
            ...
            for d, e in zip(dets_file_paths, embs_file_paths):
                ...
                if mot_result_path.exists():
                    if prompt_overwrite("MOT Result", mot_result_path, opt.ci):
                        ...
                        LOGGER.info(
                            f"Skipping MOT result generation for {d.stem} as it already exists."
                        )
                        continue
                # Submit the task to process this file pair in parallel
                tasks.append(
                    executor.submit(process_single_mot, opt, d, e, evolve_config)
                )
...
def run_trackeval(opt: argparse.Namespace) -> dict:
    ...
    Args:
        opt (Namespace): Parsed command line arguments.
    """
    seq_paths, save_dir, MOT_results_folder, gt_folder = eval_setup(
        opt, opt.val_tools_path
    )
    trackeval_results = trackeval(
        opt, seq_paths, save_dir, MOT_results_folder, gt_folder
    )
...
def parse_opt() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--conf", type=float, default=0.01, help="min confidence threshold"
    )
    parser.add_argument(
        "--iou",
        type=float,
    ...
    parser.add_argument(
        "--exp-folder-path", type=Path, help="path to experiment folder"
    )
    ...
    parser.add_argument(
        "--gsi",
        action="store_true",
        help="apply Gaussian smooth interpolation postprocessing",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--split-dataset",
        action="store_true",
        help="Use the second half of the dataset",
    )
    ...
    generate_dets_embs_parser = subparsers.add_parser(
        "generate_dets_embs", help="Generate detections and embeddings"
    )
    generate_dets_embs_parser.add_argument(
        "--source", type=str, required=True, help="file/dir/URL/glob, 0 for webcam"
    )
    generate_dets_embs_parser.add_argument(
        "--yolo-model",
        nargs="+",
    ...
    generate_mot_results_parser = subparsers.add_parser(
        "generate_mot_results", help="Generate MOT results"
    )
    generate_mot_results_parser.add_argument(
        "--yolo-model",
        nargs="+",
        type=Path,
        default=WEIGHTS / "yolov8n.pt",
    ...
    trackeval_parser = subparsers.add_parser(
        "trackeval", help="Evaluate tracking results"
    )
    trackeval_parser.add_argument(
        "--source", type=str, required=True, help="file/dir/URL/glob, 0 for webcam"
    )
    trackeval_parser.add_argument(
        "--exp-folder-path", type=Path, required=True, help="path to experiment folder"
    )
